...
collect_settings() {
    ...
    if [[ -n "$client_ip" ]]; then
        echo -e "    ${DIM}Ваш текущий IP: ${CYAN}$client_ip${NC}"
        echo ""
        echo -e "    ${WHITE}Ограничить SSH только для вашего IP?${NC}"
        echo -e "    ${YELLOW}⚠ Если у вас динамический IP или VPN — вы потеряете доступ при смене IP!${NC}"
        echo -e "    ${DIM}Введите IP для ограничения, Enter = открыть для всех, 0 = открыть для всех${NC}"
        read_input "IP админа (или Enter для всех)" "" ADMIN_IP
        [[ "$ADMIN_IP" == "0" ]] && ADMIN_IP=""
    else
        read_input "IP админа (Enter = открыть для всех)" "" ADMIN_IP
    fi
    ...
    if [[ -n "$ADMIN_IP" ]]; then
        ...
        echo ""
        echo -e "    ${DIM}SSH будет открыт для всех IP${NC}"
    fi
...
apply_protection() {
    ...
    case "${FIREWALL_MODE}" in
        ...
        "fix_issues")
            ...
            # ВАЖНО: сначала добавляем новое правило SSH, ПОТОМ удаляем старое
            # Иначе UFW активен с deny incoming и нет SSH-правила = локаут
            if [[ -n "$ADMIN_IP" ]]; then
            ...
            # Теперь безопасно удаляем старые/лишние правила
            if [[ "${PORT22_OPEN}" == "true" ]] && [[ "$SSH_PORT" != "22" ]]; then
                ufw delete allow 22/tcp 2>/dev/null
                ufw delete allow 22 2>/dev/null
            fi

            # Удаляем "открыт для всех" только если добавили ограниченное правило
            if [[ "${SSH_OPEN_ALL}" == "true" ]] && [[ -n "$ADMIN_IP" ]]; then
